...
function saveTaskFromForm() {
  alert('Кнопка Сохранить в форме нажата!');
  ...
  if(t&&h&&d&&newTaskAssignee){var task=new Task(t,newTaskType,h,d,newTaskAssignee,desc);newTaskAssignee.addTask(task);titleInput.value='';hoursInput.value='';deadlineInput.value='';if(descInput)descInput.value='';hideForm();alert('Задача добавлена!');}
...
function mousePressed() {
  ...
  if(currentManager&&!showAddForm){
    ...
    for(var b=0;b<blocks.length;b++){
      ...
      for(var j=0;j<Math.min(active.length,5);j++){var ty=226+j*34;
    ...
    for(var k=0;k<Math.min(completed.length,4);k++){var t=completed[k],ty=464+k*30;
