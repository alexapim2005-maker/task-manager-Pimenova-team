...
function setup() {
  ...
  if (managers.length === 0) {
    ...
    a.addTask(new Task('Стратегическое планирование', 'weekly', 4, 'ПН', a, 'Определить цели на квартал, распределить бюджеты'));
    a.addTask(new Task('Совещание с командой', 'weekly', 2, 'СР', a, 'Обсудить результаты недели'));
    v.addTask(new Task('Отчёт по продажам', 'weekly', 3, 'ПН', v, 'Собрать данные из CRM, подготовить графики'));
...
function drawBlock(title, tasks, x, y, color) {
  ...
  for (let i = 0; i < min(active.length, 5); i++) {
    ...
    text(t.hours + 'ч | ' + t.deadline + ' | ' + (t.assignee ? t.assignee.name.split('(')[0].trim() : ''), x+44, ty+27);
...
class Manager {
  ...
  getWeeklyHours() {
    return this.tasks.filter(t => t.type==='weekly' && t.status!=='done').reduce((s,t) => s+t.hours, 0) +
           this.tasks.filter(t => t.type==='monthly' && t.status!=='done').reduce((s,t) => s+t.hours/4, 0);
  }
...
function checkNotifications() {
  notifications = [];
  for (let m of managers) { if (m.getWeeklyHours() > 40) notifications.push({ type: 'overload', message: m.name + ': перегруз ' + m.getWeeklyHours().toFixed(1) + 'ч' }); }
}
...
function mousePressed() {
  ...
  if (currentManager && !showAddForm) {
    ...
    for (let b of blocks) {
      ...
      for (let i = 0; i < min(active.length, 5); i++) {
        let ty = 226 + i * 34;
        if (mouseX > b.x+308 && mouseX < b.x+330 && mouseY > ty+6 && mouseY < ty+26) { currentManager.removeTask(active[i]); return; }
        if (mouseX > b.x+10 && mouseX < b.x+26 && mouseY > ty+6 && mouseY < ty+22) { active[i].complete(); saveData(); checkNotifications(); return; }
      }
    }
...
function exportToXLS() {
  ...
  for (let m of managers) for (let t of m.tasks) {
