...
function mousePressed() {
  ...
  if (currentManager && !showAddForm) {
    ...
    for (let b of blocks) {
      ...
      for (let i = 0; i < min(active.length, 5); i++) {
        let ty = b.y + 50 + i * 36;
        if (mouseX > b.x+308 && mouseX < b.x+330 && mouseY > ty+2 && mouseY < ty+24) { currentManager.removeTask(active[i]); return; }
        if (mouseX > b.x+12 && mouseX < b.x+30 && mouseY > ty+2 && mouseY < ty+20) { active[i].complete(); saveData(); checkNotifications(); return; }
      }
    }
