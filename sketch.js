...
function drawTaskBlock(title, tasks, x, y, color) {
  ...
  if (active.length === 0) {
    fill('#b2bec3'); textSize(13); text('Нет задач', x+15, y+65); return;
  }
  ...
  for (let i = 0; i < min(active.length, 5); i++) {
    ...
    if (mouseX > x && mouseX < x+340 && mouseY > ty && mouseY < ty+34) {
      fill('#f8f9fa'); noStroke(); rect(x+2, ty, 336, 34, 4);
    }
    
    // Чекбокс
    stroke('#b2bec3'); strokeWeight(2);
    fill('#fff'); rect(x+12, ty+8, 16, 16, 3);
    noStroke();
    
    // Название
    fill(colors.text); textSize(12); text(t.title, x+35, ty+16);
    
    // Инфо
    fill('#636e72'); textSize(10);
    let an = t.assignee ? t.assignee.name.split('(')[0].trim() : '-';
    text(t.hours + 'ч | ' + t.deadline + ' | ' + an, x+35, ty+30);
    
    // Кнопка удаления
    let delX = x + 310;
    let delY = ty + 10;
    fill('#e74c3c'); noStroke(); rect(delX, delY, 20, 20, 4);
    fill('#fff'); textSize(14); textAlign(CENTER); text('×', delX+10, delY+16); textAlign(LEFT);
  }
}

function drawCompletedBlock(tasks, x, y) {
  fill(colors.completed); noStroke(); rect(x, y, 1040, 36, 8);
  fill('#fff'); textSize(15); textStyle(BOLD); text('✅ Завершённые задачи', x+15, y+24); textStyle(NORMAL);
  
  fill(colors.card); stroke('#e0e0e0'); rect(x, y+36, 1040, 145, 0,0,8,8);
  
  if (tasks.length === 0) {
    fill('#b2bec3'); textSize(13); text('Нет завершённых задач', x+15, y+65); return;
  }
  ...
  for (let i = 0; i < min(sorted.length, 4); i++) {
    ...
    fill(colors.ok); textSize(14); text('✓', x+15, ty+14);
    fill(colors.completed); textSize(12); textStyle(ITALIC); text(t.title, x+35, ty+14); textStyle(NORMAL);
    fill('#636e72'); textSize(9);
    text(t.hours + 'ч | Вып: ' + (t.completedDate||'?'), x+35, ty+26);
    
    // Кнопка восстановления
    let resX = x + 270, resY = ty + 4;
    fill('#dfe6e9'); noStroke(); rect(resX, resY, 35, 20, 3);
    fill(colors.text); textSize(10); textAlign(CENTER); text('↩', resX+17, resY+15); textAlign(LEFT);
    
    // Кнопка удаления
    let delX = x + 315, delY = ty + 4;
    fill('#e74c3c'); noStroke(); rect(delX, delY, 24, 20, 3);
    fill('#fff'); textSize(12); textAlign(CENTER); text('×', delX+12, delY+15); textAlign(LEFT);
  }
}

function drawStats() {
  let x = 30, y = 610;
  fill(colors.card); stroke('#e0e0e0'); rect(x, y, 1040, 75, 8);
  fill(colors.text); textSize(16); textStyle(BOLD);
  text('📊 Нагрузка: ' + currentManager.name.split('(')[0].trim(), x+20, y+28); textStyle(NORMAL);
  
  let wh = currentManager.getWeeklyHours();
  let mh = currentManager.getMonthlyHours();
  
  drawBar(x+20, y+40, 300, 'Неделя', wh, 40, wh > 40 ? colors.danger : wh > 35 ? colors.warning : colors.ok);
  drawBar(x+370, y+40, 300, 'Месяц', mh, 160, mh > 160 ? colors.danger : mh > 140 ? colors.warning : colors.ok);
  
  fill(colors.text); textSize(13);
  text('Разовые: ' + currentManager.getOnetimeHours() + 'ч | Свободно: ' + max(0,40-wh) + 'ч', x+720, y+55);
}
...
function drawBar(x, y, w, label, val, max, color) {
  ...
  fill(color); rect(x, y+8, w * min(val/max, 1), 20, 10);
}

function drawAddButton() {
  fill('#00b894'); noStroke(); rect(850, 705, 220, 44, 22);
  fill('#fff'); textSize(16); textAlign(CENTER); text('+ Новая задача', 960, 734); textAlign(LEFT);
}
...
function mousePressed() {
  ...
  if (mouseY > 95 && mouseY < 129) {
    ...
    if (mouseX > 180 && mouseX < 320) { activeView = 'team'; return; }
    if (mouseX > 330 && mouseX < 470) { activeView = 'calendar'; return; }
  }
  ...
  for (let i = 0; i < managers.length; i++) {
    if (mouseX > 130+i*200 && mouseX < 315+i*200 && mouseY > 140 && mouseY < 170) {
      currentManager = managers[i]; return;
    }
  }
  ...
  if (showAddForm) {
    ...
    for (let i = 0; i < 3; i++) {
      if (mouseX > 320+i*90 && mouseX < 400+i*90 && mouseY > 680 && mouseY < 710) {
        newTaskType = types[i]; return;
      }
    }
    // Ответственный
    for (let i = 0; i < managers.length; i++) {
      if (mouseX > 170+i*120 && mouseX < 280+i*120 && mouseY > 718 && mouseY < 746) {
        newTaskAssignee = managers[i]; return;
      }
    }
    // Сохранить
  ...
  if (currentManager) {
    ...
    for (let b of blocks) {
      ...
      for (let i = 0; i < min(active.length, 5); i++) {
        ...
        // Удаление
        if (mouseX > b.x+310 && mouseX < b.x+330 && mouseY > ty+10 && mouseY < ty+30) {
          currentManager.removeTask(active[i]); return;
        }
        
        // Чекбокс
        if (mouseX > b.x+12 && mouseX < b.x+28 && mouseY > ty+8 && mouseY < ty+24) {
    ...
    for (let i = 0; i < min(comp.length, 4); i++) {
      ...
      // Удаление
      if (mouseX > 345 && mouseX < 369 && mouseY > ty+4 && mouseY < ty+24) {
        currentManager.removeTask(comp[i]); return;
      }
      
      // Восстановление
      if (mouseX > 300 && mouseX < 335 && mouseY > ty+4 && mouseY < ty+24) {
        comp[i].reopen(); saveData(); checkNotifications(); return;
      }
    }
  }
}
...
function keyPressed() {
  if (showAddForm) {
    if (keyCode === BACKSPACE) {
      if (mouseX > 50 && mouseX < 300 && mouseY > 680 && mouseY < 710) newTaskTitle = newTaskTitle.slice(0, -1);
      else if (mouseX > 600 && mouseX < 660 && mouseY > 680 && mouseY < 710) newTaskHours = newTaskHours.slice(0, -1);
      else if (mouseX > 670 && mouseX < 780 && mouseY > 680 && mouseY < 710) newTaskDeadline = newTaskDeadline.slice(0, -1);
      ...
      if (mouseX > 50 && mouseX < 300 && mouseY > 680 && mouseY < 710) newTaskTitle += key;
      else if (mouseX > 600 && mouseX < 660 && mouseY > 680 && mouseY < 710) newTaskHours += key;
      else if (mouseX > 670 && mouseX < 780 && mouseY > 680 && mouseY < 710) newTaskDeadline += key;
    }
  }
}
...
function createTestData() {
  ...
  a.tasks.push(new Task('Стратегическое планирование', 'weekly', 4, 'ПН', a));
  a.tasks.push(new Task('Совещание с командой', 'weekly', 2, 'СР', a));
  v.tasks.push(new Task('Отчёт по продажам', 'weekly', 3, 'ПН', v));
  v.tasks.push(new Task('Презентация для клиента', 'onetime', 6, '28.06.2026', v));
  va.tasks.push(new Task('Анализ рынка', 'weekly', 4, 'ПН', va));
  va.tasks.push(new Task('Обновление базы данных', 'onetime', 8, '27.06.2026', va));
  saveData();
}
...
function exportToXLS() {
  ...
  xls += '<Row><Cell><Data ss:Type="String">Сотрудник</Data></Cell><Cell><Data ss:Type="String">Задача</Data></Cell><Cell><Data ss:Type="String">Тип</Data></Cell><Cell><Data ss:Type="String">Часы</Data></Cell><Cell><Data ss:Type="String">Дедлайн</Data></Cell><Cell><Data ss:Type="String">Статус</Data></Cell></Row>';
  for (let m of managers) {
    for (let t of m.tasks) {
      let type = t.type==='weekly'?'Еженедельная':t.type==='monthly'?'Ежемесячная':'Разовая';
      let status = t.status==='done'?'Выполнена':'В работе';
      xls += '<Row><Cell><Data ss:Type="String">'+m.name+'</Data></Cell><Cell><Data ss:Type="String">'+t.title+'</Data></Cell><Cell><Data ss:Type="String">'+type+'</Data></Cell><Cell><Data ss:Type="Number">'+t.hours+'</Data></Cell><Cell><Data ss:Type="String">'+t.deadline+'</Data></Cell><Cell><Data ss:Type="String">'+status+'</Data></Cell></Row>';
    }
  }
...
function drawTeamDashboard() {
  ...
  for (let m of managers) {
    ...
    textAlign(CENTER);
    fill(wh>40?colors.danger:colors.text); text(wh.toFixed(1)+'ч', x+300, y+30);
    fill(fw>0?colors.ok:'#636e72'); text(fw.toFixed(1)+'ч', x+450, y+30);
    fill(mh>160?colors.danger:colors.text); text(mh.toFixed(1)+'ч', x+600, y+30);
    fill(fm>0?colors.ok:'#636e72'); text(fm.toFixed(1)+'ч', x+750, y+30);
    textAlign(LEFT);
    fill('#dfe6e9'); rect(x+850, y+14, 200, 12, 6);
    let lc = pct>100?colors.danger:pct>80?colors.warning:colors.ok;
    fill(lc); rect(x+850, y+14, 200*min(pct/100,1), 12, 6);
...
function drawCalendarView() {
  ...
  // Выбор менеджера в календаре
  if (mousePressed) {
    for (let i = 0; i < managers.length; i++) {
      if (mouseX > 380+i*200 && mouseX < 560+i*200 && mouseY > 138 && mouseY < 166) {
